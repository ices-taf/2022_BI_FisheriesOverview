...
guild2 <- guild2 %>% filter(FisheriesGuild != "elasmobranch")
guild3 <- guild2 %>% filter(Year > 1980)
guild3 <- guild3 %>% filter(Year < 2022)
...
guild3 <- guild3 %>% filter(Year < 2022)
guild3 <- guild3 %>% filter(FisheriesGuild != "elasmobranch")
plot_guild_trends(guild3, cap_year, cap_month,return_data = FALSE )
dem <- guild3 %>% filter(FisheriesGuild == "demersal")
dem <- dem %>% filter(Year> 1981)
guild3 <- guild3 %>% filter(FisheriesGuild!= "demersal")
guild4 <- rbind(guild3, dem)
